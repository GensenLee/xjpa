...
import com.glee.xjpa.configuration.configsource.AnnotationDataSourceBeanConfig;
import com.glee.xjpa.configuration.configsource.ScanListXjpaRepositoryBeanConfig;
import com.glee.xjpa.configuration.configsource.XjpaRepositoryBeanConfig;
import com.glee.xjpa.exception.XjpaException;
import com.glee.xjpa.lifecycle.CacheRepositoryPackageDataSourceMapper;
import com.glee.xjpa.repository.StandardJpaRepository;
import com.glee.xjpa.util.ReflectionsUtil;
...
public class SpringRepositoryGlobalConfig implements RepositoryGlobalConfig {
    ...
    public void refresh() {
        ...
        xjpaRepositoryBeanConfigList.addAll(beanProvider.stream().toList());

        List<Map<String[], String>> scanResultList = xjpaRepositoryBeanConfigList.stream()
                .map(XjpaRepositoryBeanConfig::scanPackages).toList();
        ...
        repositoryPackageDataSourceMapper = new CacheRepositoryPackageDataSourceMapper(packageDataSourceMapping);
    }
}
